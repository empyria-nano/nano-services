...
const ENV_KEYS = ['EMPYRIA_PROMETHEUS_PORT']
...
describe('apps/acme env.js', () => {
	test('defaults EMPYRIA_PROMETHEUS_PORT to 3031', async () => {
		const { env } = await loadEnv()
		expect(env.EMPYRIA_PROMETHEUS_PORT).toBe(3031)
	})
	...
	test('coerces an explicitly-set string port to a number', async () => {
		...
		const { env } = await loadEnv({ EMPYRIA_PROMETHEUS_PORT: '5050' })
		expect(env.EMPYRIA_PROMETHEUS_PORT).toBe(5050)
